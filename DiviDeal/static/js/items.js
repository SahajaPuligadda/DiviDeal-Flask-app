...
function changed(evt) {
    ...
    if (evt.target.checked) {
	  ...
	  if(document.querySelector('.active').innerHTML == "Shares"){
        evt.target.parentElement.parentElement.nextElementSibling.firstElementChild.value = 1;
      }
    }
    else {
      //var par = evt.path[3];
...
function Checkvalue(){
	if(document.querySelectorAll(".checkbox-1").length <= 2){
      window.alert("Add atleast 2 participants")
      return;
    }
...
function make_equal(){
  ...
  var c_len = total_consumers.length;
  var total_amount = document.querySelector("#InputAmount").valueAsNumber;
  var split = (total_amount/c_len).toFixed(2);
  console.log(total_amount);
  console.log(split);
  var menuName = document.querySelector('.active').innerHTML;
