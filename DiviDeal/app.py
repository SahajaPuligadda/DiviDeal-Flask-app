...
app.config['MAIL_USERNAME'] = 'DiviDeal919@gmail.com'
app.config['MAIL_PASSWORD'] = 'mvsykgrnqtuxeltu'
...
def create():
    if request.method == "POST":
        ...
        # plink = plink.replace(" ","")
        msg = Message(subject="DiviDeal: Your pool-link", sender='DiviDeal919@gmail.com', recipients=[email])
        msg.html = render_template("mymail.html", plink=plink)
        mail.send(msg)
...
def add_item(pool_link):
    ...
    if request.method == "POST":
        ...
        consumer_amount = adjust_amts(consumer_amount, amount)
...
def edit_item(pool_link, item_id):
    ...
    if request.method == "POST":
        ...
        con_amt = [x for x in consumer_amount if float(x) > 0]
        update_item(pool_id, item_id, description, date, amount, payer_name, consumer, con_amt, expense_type, coltwo,
                    item, consumers)
        return redirect(url_for("pool_items", pool_link=pool_link))

    return render_template('edit_item.html', item=item, consumers=consumers, excludedp=notpaidby,
                           notconsumer=notconsumer, consumer=consumer, expense_type=expense_type,
                           col2_values=col2_values, participant_names=participants, pool_link=pool_link)
