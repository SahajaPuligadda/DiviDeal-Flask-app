...
def update_participant(pot_id,paidby,amount,consumers,consumer_amount):
    ...
    updated_net = round(updated_paid - paid_values[1],2)
    ...
    for i in range(len(consumers)):
        ...
        updated_net = round(consumed_values[0] - updated_consumed,2)
...
def adjust_amts(consumer_amount,amount):
    total_camts = 0
    for con in consumer_amount:
        total_camts+= float(con)
    consumer_amount[0] = str(float(consumer_amount[0])+float(amount)-total_camts)
